...
At the same time, I believe a blog website should remove as much clutter that interferes with reading as possible, keeping only the pure content—in other words, it should be fast and pure.刚好Astro is famously fast, and in the future, making small style adjustments no longer requires running around everywhere—just directly using Astro. Thinking about it, it seems quite perfect.
